    ...
    if (function_exists('acf_add_local_field_group')) {
      acf_add_local_field_group([
        ...
        'fields'   => [
          ...
          [
            ...
            'type'         => 'textarea',
            'required'     => 0,
            'instructions' => 'Enter a short paragraph to display under the contact info.',
            'rows'         => 3,
            'new_lines'    => 'br',
          ],
        ],
      ...
      acf_add_local_field_group([
        ...
        'fields' => [
          ...
          [
            'key'           => 'field_el_loc_type_filter_order',
            'label'         => 'Filter & list order',
            'name'          => 'filter_list_order',
            'type'          => 'number',
            'instructions'  => 'Lower numbers appear first in the map filter bar and location sections. Leave empty (or 0) to place this type after all numbered types, sorted alphabetically by name.',
            'required'      => 0,
            'min'           => 0,
            'step'          => 1,
            'default_value' => '',
  ...
  public static function get_all_location_types()
  {
    ...
    if (!is_wp_error($location_types) && !empty($location_types)) {
      foreach ($location_types as $term) {
        ...
        $order = (int) get_field('filter_list_order', 'location-type_' . $term->term_id);
        $result[] = [
          'term'             => $term,
          'icon'             => $icon,
          '_filter_list_order' => $order,
        ];
      }

      usort($result, static function (array $a, array $b): int {
        $oa = $a['_filter_list_order'] ?? 0;
        $ob = $b['_filter_list_order'] ?? 0;
        if ($oa === 0 && $ob === 0) {
